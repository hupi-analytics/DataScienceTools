...
import org.apache.commons.math3.distribution._
 ...
 package io.hupi.datascience_tools {
	...
	object Outliers{
		...
		/* function : outliersGrubbs(variable : RDD[Double], alpha : Double = 0.05) : RDD[Double]
		 ...
		 * @return a RDD[Double] with max or/and min if their test are rejected (so if there are outliers)
		 *         if both are not outlier, it returns an empty RDD[Double]
		 */
		def outliersGrubbs(variable : RDD[Double], alpha : Double = 0.05) : List[Double] = {
		...
		/* function : outliersTukey(variable : RDD[Double], power : String = "strong") : RDD[Double]                           
		 ...
		 */
		def outliersTukey(variable : RDD[Double], power : String = "strong") : RDD[Double] = {
		 ...
		 * @return a RDD[Double] which contains the outliers
		 */
		def outliers3S(variable : RDD[Double], power : String = "strong") : RDD[Double] = {
		...
		/* function : outliersESD(variable : RDD[Double], nbOutliers : Int = 10, alpha : Double = 0.05) : List[Double]    
		 ...
		 */
		def outliersESD(variable : RDD[Double], nbOutliers : Int = 10, alpha : Double = 0.05) : List[Double] = {
		...
		/* function : outliers(variable : RDD[Double], method : String = "esd", alpha : Double = 0.05,
							   nbOutliers : Int = 10, power : String = "strong"): RDD[Double]    
		 ...
		 *
		 * @param variable: input data, it's a RDD of Double 
		 * @param method: one of the four method available : ESD, 3S, Tukey
		 * @param alpha: test confidence (0.05 by default) between 0 and 1 (alpha in ]0,1[) for ESD 
		 * @param nbOutliers: number of test to do (and point) for ESD
		...
		def outliers(
		  ...
		  method.toLowerCase match{
			case "esd" => outliersESD(variable, nbOutliers, alpha )
			case "3s" => outliers3S(variable, power)
			case "tukey" => outliersTukey(variable, power)
			//case "grubbs" => outliersGrubbs(variable, alpha)
		...
		*/
		...
		/* function : replaceOutliers(variable: RDD[Double], outliers : RDD[Double], replace: String = "nothing" ) : RDD[Double] 
		 ...
		 */
		def replaceOutliers(variable: RDD[Double], outliers : RDD[Double], replace: String = "nothing" ) : RDD[Double] = {
